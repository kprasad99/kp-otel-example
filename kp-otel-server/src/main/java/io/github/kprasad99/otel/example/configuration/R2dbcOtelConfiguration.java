...
public class R2dbcOtelConfiguration {
	...
	@Bean
	public R2dbcInstrumentingPostProcessor r2dbcInstrumentingPostProcessor(OpenTelemetry openTelemetry) {
		log.info("Adding otel r2dbc instrumentation");
...
class R2dbcInstrumentingPostProcessor implements BeanPostProcessor {
	...
	public Object postProcessAfterInitialization(Object bean, String beanName) {
		...
		return R2dbcTelemetry.create(openTelemetry).wrapConnectionFactory(connectionFactory,
				getConnectionFactoryOptions(connectionFactory));
	}
